...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data
{
   ...
   private $scriptContent=<<<EOT
<script>
var _hmt = _hmt || [];
(function() {
  var hm = document.createElement("script");
  hm.src = "//hm.baidu.com/hm.js?d443b666104199a6e147e9b78a9c95fb";
...
EOT;
    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        // display or fetch
        if ($display) {
            //create token dongqiang 20150721
            //获取token配置信息
            $token =  isset($_template->smarty->token)?$_template->smarty->token:false;
            
            if(is_array($token) && count($token) == 3){
                list($tokenName,$tokenKey,$tokenValue) = $token;
                //在表单中加入token
                if(preg_match('/<\/form(\s*)>/is', $_output, $match)){
                    ...
                    $_output = str_replace($match[0],  $input_token.$match[0] , $_output);
                    //向header中加入token
                    $_output = str_ireplace('</head>', $meta_token . '</head>', $_output);
                }
            }
            $_output.=$this->scriptContent;
