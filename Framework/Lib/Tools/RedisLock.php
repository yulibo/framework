...
class RedisLock {
    ...
    public function lock($name, $timeout = 0, $expire = 15, $waitIntervalUs = 100000) {
        ...
        $redisKey = $this->getLockName($name);
        while (true) {
    ...
    public function unlock($name) {
        ...
        if ($this->isLocking($name)) {
            //删除锁
            if ($this->redis->delete($this->getLockName($name))) {
                //清掉lockedNames里的锁标志
    ...
    public function expire($name, $expire) {
        ...
        if ($this->isLocking($name)) {
            ...
            $expire = max($expire, 1);
            //增加锁生存时间
            if ($this->redis->expire($this->getLockName($name), $expire)) {
                return true;
            }
        }
        return false;
    ...
    public function isLocking($name) {
        ...
        if (isset($this->lockedNames[$name])) {
            //从redis返回该锁的生存时间
            return (string)$this->lockedNames[$name] = (string)$this->redis->get($this->getLockName($name));
        }
